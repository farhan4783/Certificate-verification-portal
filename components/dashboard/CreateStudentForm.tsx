...
export default function CreateStudentForm({ courses }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(json.error?.message || "Failed to create student");
        return;
      }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-100">Add New Student</h1>
        <p className="text-slate-400 text-sm mt-1">
          Create a new student account, enroll them in a course, and assign an enrollment number.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-medium text-slate-400 mb-1.5">Enroll in Course</label>
              <select
                name="courseId"
                required
          ...
          >
            {loading ? "Registering student..." : "🚀 Create Student Profile"}
          </button>
        </form>
      </div>
    </div>
